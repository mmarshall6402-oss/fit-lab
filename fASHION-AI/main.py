...
# server-to-server, so no browser origins are allowed by default; set
...
class ClothingItem(BaseModel):
    id: int
    description: str
    # Optional photo of the actual item, base64-encoded. The trained
    # classifier only ever saw image embeddings during /add-fit, so it
    # can only be applied here when a real photo is available too -
    # otherwise its input would be out-of-distribution (text vs image
    # embeddings occupy different regions of CLIP's shared space even
...
def get_item_embedding(item: ClothingItem):
    """Returns (embedding, is_image). Prefers a real photo so the
    embedding matches what the classifier was trained on; falls back to
    the text description (kept out of the classifier, see below)."""
    if item.image_base64:
        try:
            img_bytes = base64.b64decode(item.image_base64)
            img = Image.open(io.BytesIO(img_bytes)).convert("RGB")
            return model.encode(img), True
        except Exception as e:
            raise HTTPException(status_code=400, detail=f"Failed to decode image for item {item.id}: {str(e)}")
    return model.encode(item.description), False
...
async def add_fit(
    ...
    # Check if a single full fit photo was provided
    if single_full_fit_image and single_full_fit_image.filename:
        fit_emb = get_image_embedding(single_full_fit_image)
        # Duplicate the vector across all 4 vector slots to match the required 2048-dim input array shape
        outfit_vector = np.concatenate([fit_emb, fit_emb, fit_emb, fit_emb])
    else:
        # Fall back to separate item verification constraints
...
# =====================================================================
# VISUAL OUTFIT GENERATION ENGINE
# =====================================================================
...
async def generate_outfit_from_top(inventory_json: str, file: UploadFile = File(...), _: None = Depends(require_api_key)):
    ...
    try:
        ...
        pants_data = [(p, *get_item_embedding(p)) for p in inventory.pants]
        ranked_pants = [
            {
                "id": p.id,
                "description": p.description,
                "match_score": float(util.cos_sim(top_embedding, emb)),
            }
            for p, emb, _is_image in pants_data
        ]
        ranked_pants.sort(key=lambda x: x["match_score"], reverse=True)
        pants_emb_by_id = {p.id: (emb, is_image) for p, emb, is_image in pants_data}

        ranked_shoes = []
        for s in inventory.shoes:
            shoe_emb, shoe_is_image = get_item_embedding(s)
            best_pants = ranked_pants[0] if ranked_pants else None
            best_pants_emb, best_pants_is_image = (
                pants_emb_by_id[best_pants["id"]] if best_pants else (None, False)
            )

            # Only trust the trained classifier when every slot it's
            # being fed is a real photo - the same modality it trained
            # on. Otherwise fall back to plain cosine similarity rather
            # than scoring text embeddings with an image-trained model.
            if clf and shoe_is_image and best_pants_is_image:
                combined = np.concatenate(
                    [top_embedding, best_pants_emb, shoe_emb, np.zeros(512)]
                ).reshape(1, -1)
                score = float(clf.predict_proba(combined)[0][1])
            else:
