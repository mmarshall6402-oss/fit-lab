...
public class OutfitScoringService {
    ...
    private static final Set<String> NEUTRAL_COLORS =
    ...
    private double maxPossiblePairScore(Item a, Item b) {
        ...
        return maxColorOverlap * TagMatcher.W_COLOR + maxVibeOverlap * TagMatcher.W_VIBE;
    }
    ...
    public List<String> reasons(Item shirt, Item bottom, Item shoes) {
        ...
        if (shirt.getColors().size() >= 3) {
            bottom.getColors().stream()
                    .filter(NEUTRAL_COLORS::contains)
                    .findFirst()
